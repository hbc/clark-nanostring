...
We can see samples that are from the non-progressing samples tend to have
a lower binding density and the samples that are from the progressing samples
tend to have a higher binding density. If there isn't a good biological explanation,
that is bad because we have some kind of technical batch effect going on.
...
each sample. We will use that to normalize the counts.
...
I thought that the normalization factor would be related to the binding density
somehow, but it isn't. Despite the binding density being super batch-affected,
none of the other metrics seem affected by it.
...
plotBD = function(eset, metadata) {
  pdat = pData(eset) %>%
    ...
    left_join(metadata, by=c("rowname"="sample"))
  pdat$pcounted = pdat$FovCounted/pdat$FovCount * 100
  ggplot(pdat, aes(normfactor, BindingDensity, color=condition)) +
    geom_point() +
    scale_y_continuous(expand = c(0,0)) +
    expand_limits(y = c(0,1.05 * max(pdat$BindingDensity))) +
    ylab("Binding density") +
    xlab("nomalization factor") +
    geom_hline(yintercept=0.05, color="red") +
    geom_hline(yintercept=2.25, color="red")
}
plotBD(eset, metadata)
...
We'll also pick genes that have a CV on the lower end. We'll set a cutoff of 1.75:
```{r hk-cv-selection}
